...
[true, false].forEach((useCustomMetadata) => {
    suite(`Import Tracker (${useCustomMetadata ? 'with custom metadata' : 'without custom metadata'})`, async () => {
        ...
        class Reporter {
            ...
            public static async expectHashes(
                ...
                assert.deepEqual(
                    ...
                    `Hashes not sent correctly, expected ${JSON.stringify(expected)} but got ${JSON.stringify(
                        properties
                    )}`
                );
            }
        ...
        setup(() => {
            ...
            importTracker = new ImportTracker([], 0);
        });
        teardown(() => {
            sinon.restore();
        ...
        test('Open document', async () => {
            ...
            const nb = createMockedNotebookDocument(useCustomMetadata, [
