...
import { BaseDisposable, DisposableStore, SafeDisposable, combinedDisposable, toDisposable } from './lifecycle';
...
export const None: Event<any> = () => BaseDisposable.None;
 ...
 */
export function defer(event: Event<unknown>, disposable?: DisposableStore | IDisposable[]): Event<void> {
 ...
 * @param disposable A disposable store to add the new EventEmitter to.
 */
export function map<I, O>(event: Event<I>, map: (i: I) => O, disposable?: DisposableStore | IDisposable[]): Event<O> {
    return snapshot(
...
export function forEach<I>(
    event: Event<I>,
    each: (i: I) => void,
    disposable?: DisposableStore | IDisposable[]
): Event<I> {
    return snapshot(
...
function snapshot<T>(event: Event<T>, disposable: DisposableStore | IDisposable[] | undefined): Event<T> {
...
export function split<T, U>(
    ...
    return [filter(event, isT, disposable), filter(event, (e) => !isT(e), disposable) as Event<U>];
}
...
export class EventDeliveryQueue {
    ...
    constructor(private readonly _onListenerError: (e: any) => void = noop) {}
...
class PrivateEventDeliveryQueue extends EventDeliveryQueue {
    override clear<T>(_emitter: Emitter<T>): void {
...
export class Relay<T> implements IDisposable {
    ...
    private inputEventListener: IDisposable = BaseDisposable.None;
