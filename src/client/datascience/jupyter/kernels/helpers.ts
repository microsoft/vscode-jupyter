...
export function getKernelId(spec: IJupyterKernelSpec, interpreter?: PythonEnvironment, remoteBaseUrl?: string) {
    ...
    const kernelRegistrationKind = isKernelRegisteredByUs(spec);
    ...
    if (kernelRegistrationKind && specName.includes(autoGeneratedKernelNameIdentifier)) {
        ...
        if (kernelRegistrationKind === 'newVersionUserKernelSpec') {
        ...
        case 'startUsingPythonInterpreter':
            if (
                ...
                if (
                    ...
                    isKernelRegisteredByUs(kernelConnection.kernelSpec) === 'newVersionUserKernelSpec'
                ) {
 ...
 * When dealing with non-raw kernels, we register kernelspecs in a global location.
 ...
 * `newVersionUserKernelSpec`
 */
export function isKernelRegisteredByUs(
    kernelSpec: IJupyterKernelSpec
): 'oldVersion' | 'newVersion' | 'newVersionUserKernelSpec' | undefined {
    ...
    const isOriginalKernelSpecCreatedByUser = isDefaultKernelSpec({ ...kernelSpec, name: originalName })
    ...
    if (kernelSpec.name.includes(autoGeneratedKernelNameIdentifier)) {
        return isOriginalKernelSpecCreatedByUser ? 'newVersionUserKernelSpec' : 'newVersion';
    }
...
export function createInterpreterKernelSpec(
    ...
    return new JupyterKernelSpec(defaultSpec, specFile, interpreter?.path, 'newVersion');
}
            ...
            if (spec) {
                ...
                if (
                    ...
                    (isKernelRegisteredByUs(spec) === 'newVersion' ||
                        isKernelRegisteredByUs(spec) === 'newVersionUserKernelSpec') &&
