...
import type { KernelSpecManager, SessionManager, KernelManager, Session, ContentsManager } from '@jupyterlab/services';
...
import {
    JupyterSession,
    JupyterSessionStartError,
    getRemoteSessionOptions,
    waitForIdleOnSession
} from './jupyterSession';
    ...
    public async create(options: KernelSessionCreationOptions): Promise<IJupyterKernelSession> {
        ...
        try {
            ...
            const sessionManager = await raceCancellationError(
                options.token,
                this.sessionManagerFactory.create(connection)
            );
            ...
            const wrapperSession = new JupyterSession(
                session,
                options.resource,
            ...
            this.asyncDisposables.push(wrapperSession);
