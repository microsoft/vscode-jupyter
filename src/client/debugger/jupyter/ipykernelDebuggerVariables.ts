...
import { DataFrameLoading, GetVariableInfo, Telemetry } from '../../datascience/constants';
import {
...
@injectable()
export class IpykernelDebuggerVariables implements IConditionalJupyterVariables, DebugAdapterTracker {
    ...
    constructor(
        @inject(IDebuggingManager) private debuggingManager: IDebuggingManager,
    ...
    public async getDataFrameRows(
        ...
        if (targetVariable.columns === undefined) {
    ...
    private async evaluate(code: string, frameId?: number): Promise<any> {
        ...
        if (session) {
            const results = await session.customRequest('evaluate', {
                expression: code,
                frameId: frameId,
                context: 'repl',
    ...
    private getSession(): DebugSession | undefined {
