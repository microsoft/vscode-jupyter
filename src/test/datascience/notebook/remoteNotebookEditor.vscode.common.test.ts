...
suite('Remote Execution @kernelCore', function () {
    ...
    suiteSetup(async function () {
        ...
        sinon.restore();
        storageFile = Uri.joinPath(
            api.serviceContainer.get<IExtensionContext>(IExtensionContext).globalStorageUri,
    ...
    test('MRU and encrypted storage should be updated with remote Uri info', async function () {
        const previousContents = await fs
        ...
        // Wait for MRU to get updated
        await waitForCondition(async () => fsWriteSpy.called, 5_000, 'Storage not updated');
        let newContents: unknown[];
        await waitForCondition(
            async () => {
                newContents = await fs
                    .readFile(storageFile)
                    .then((b) => JSON.parse(b.toString()))
                    .catch(() => []);
                assert.notDeepEqual(newContents, previousContents, 'MRU not updated');
                return true;
            },
            5_000,
            () => `MRU not updated, previously ${JSON.stringify(previousContents)}, now ${JSON.stringify(newContents)}`
        );
    });
