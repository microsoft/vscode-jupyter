    ...
    setDisposableTracker(
        new (class implements IDisposableTracker {
            ...
            setParent(child: IDisposable, _parent: IDisposable | null): void {
                if (child && child !== BaseDisposable.None) {
                    try {
            ...
            markAsDisposed(disposable: IDisposable): void {
                if (disposable && disposable !== BaseDisposable.None) {
                    try {
...
export function dispose<T extends IDisposable>(arg: T | Iterable<T> | undefined): any {
    if (Iterable.is(arg)) {
        ...
        if (errors.length === 1) {
            ...
            throw new Error(
                'Encountered errors while disposing of store' + errors.map((e, i) => `\n  (${i + 1}) ${e}`)
            );
        }
...
export class DisposableStore implements IDisposable {
     ...
     * Add a new {@link IDisposable disposable} to the collection.
     */
    public push<T extends IDisposable>(o: T): T {
        return this.add(o);
    }
}
...
export abstract class BaseDisposable implements IDisposable {
    ...
    protected _register<T extends IDisposable>(o: T): T {
        if ((o as unknown as BaseDisposable) === this) {
            throw new Error('Cannot register a disposable on itself!');
        }
