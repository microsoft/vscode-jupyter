...
function createCancellationError() {
    class InternalCancellationError extends Error {
        constructor() {
            super('Cancelled');
            this.name = 'InternalCancellationError';
        }
    }
    try {
...
export class Delayer<T> implements IDisposable {
    ...
    cancel(): void {
        ...
        if (this.completionPromise) {
            this.doReject?.(createCancellationError());
            this.completionPromise = null;
        }
    }
...
export function disposableTimeout(handler: () => void, timeout = 0): IDisposable {
    ...
    return {
        dispose: () => clearTimeout(timer as any)
    };
}
...
export class RunOnceScheduler implements IDisposable {
    ...
    cancel(): void {
        if (this.isScheduled()) {
            clearTimeout(this.timeoutToken as any);
            this.timeoutToken = -1;
        }
    }
...
export class ProcessTimeRunOnceScheduler {
    ...
    cancel(): void {
        if (this.isScheduled()) {
            clearInterval(this.intervalToken as any);
            this.intervalToken = -1;
        }
    }
...
export class DeferredPromise<T> {
    ...
    public cancel() {
        return this.error(createCancellationError());
    }
}
...
export class AsyncIterableObject<T> implements AsyncIterable<T> {
    ...
    public static EMPTY = AsyncIterableObject.fromArray<unknown>([]);
...
export function createCancelableAsyncIterable<T>(
    ...
    return new CancelableAsyncIterableObject<T>(source, async (emitter) => {
        const subscription = source.token.onCancellationRequested(() => {
            subscription.dispose();
            source.dispose();
            emitter.reject(createCancellationError());
        });
        try {
