...
[true, false].forEach((useCustomMetadata) => {
    suite(
        `Cell Execution Message Handler (${useCustomMetadata ? 'with custom metadata' : 'without custom metadata'})`,
        async () => {
            ...
            function createNotebook(cells: NotebookCellData[]) {
                const notebook = createMockedNotebookDocument(useCustomMetadata, cells);
