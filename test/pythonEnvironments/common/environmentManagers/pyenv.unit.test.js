...
const platformUtils = require("../../../../client/common/utils/platform");
const fileUtils = require("../../../../client/pythonEnvironments/common/externalDependencies");
const pyenv_1 = require("../../../../client/pythonEnvironments/common/environmentManagers/pyenv");
suite('Pyenv Identifier Tests', () => {
    const home = platformUtils.getUserHomeDir() || '';
    let getEnvVariableStub;
    let pathExistsStub;
    let getOsTypeStub;
    setup(() => {
    ...
    testData.forEach(({ testTitle, interpreterPath, pyenvEnvVar, osType }) => {
        test(`The environment variable is set to ${testTitle} on ${osType}, and the interpreter path is in a subfolder of the pyenv folder`, async () => {
            ...
            const result = await (0, pyenv_1.isPyenvEnvironment)(interpreterPath);
...
suite('Pyenv Versions Parser Test', () => {
    ...
    testData.forEach((data) => {
        test(`Parse pyenv version [${data.input}]`, async () => {
            assert.deepStrictEqual((0, pyenv_1.parsePyenvVersion)(data.input), data.expectedOutput);
        });
    });
});
suite('Pyenv Shims Dir filter tests', () => {
    ...
    test('isPyenvShimDir: valid case', () => {
        assert.deepStrictEqual((0, pyenv_1.isPyenvShimDir)(path.join(pyenvRoot, 'shims')), true);
    });
    test('isPyenvShimDir: invalid case', () => {
        assert.deepStrictEqual((0, pyenv_1.isPyenvShimDir)(__dirname), false);
    });
});
//# sourceMappingURL=pyenv.unit.test.js.map