...
import { IJupyterConnection, IJupyterKernel, IJupyterKernelSpec, IJupyterPasswordConnect } from '../types';
...
export class JupyterConnectionManager {
    ...
    protected contentsManager: ContentsManager | undefined;
    protected connInfo: IJupyterConnection | undefined;
    protected serverSettings: ServerConnection.ISettings | undefined;
    ...
    constructor(
        protected jupyterPasswordConnect: IJupyterPasswordConnect,
        _config: IConfigurationService,
        protected failOnPassword: boolean | undefined,
        protected outputChannel: IOutputChannel,
        protected configService: IConfigurationService,
        protected readonly appShell: IApplicationShell,
        protected readonly stateFactory: IPersistentStateFactory
    ) {
    ...
    protected async getServerConnectSettings(connInfo: IJupyterConnection): Promise<ServerConnection.ISettings> {
    ...
    protected async secureConnectionCheck(connInfo: IJupyterConnection): Promise<void> {
        ...
        let serverSecurePromise = JupyterConnectionManager.secureServers.get(connInfo.baseUrl);

        if (serverSecurePromise === undefined) {
            serverSecurePromise = this.insecureServerWarningPrompt();
            JupyterConnectionManager.secureServers.set(connInfo.baseUrl, serverSecurePromise);
        }
