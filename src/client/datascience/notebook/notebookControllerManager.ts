...
export class NotebookControllerManager implements INotebookControllerManager, IExtensionSyncActivationService {
                ...
                .finally(() => {
                    ...
                    this.interpreters.onDidChangeInterpreters(
                        () => {
                            ...
                            timer = setTimeout(
                                () =>
                                    this.loadNotebookControllersImpl(false, false, 'ignoreCache').catch((ex) =>
                                        console.error(
    ...
    private reloadControllersAfterChangingRemote() {
        ...
        const refreshRemoteKernels = async () => {
            if (this.isLocalLaunch) {
                ...
                if (!wasLocal) {
                    void this.loadNotebookControllersImpl(false, true, 'ignoreCache');
                }
                wasLocal = true;
                return;
            }
    ...
    private async getKernelConnectionMetadata(
        ...
        if (this.isLocalLaunch || listLocalNonPythonKernels || !listRemoteKernels) {
