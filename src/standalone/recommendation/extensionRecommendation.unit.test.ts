...
[true, false].forEach((useCustomMetadata) => {
    ...
    suite(
        `Extension Recommendation (${useCustomMetadata ? 'with custom metadata' : 'without custom metadata'})`,
        () => {
                ...
                ['csharp', 'fsharp', 'powershell'].forEach((languageToBeTested) => {
                    suite(`Notebook language '${languageToBeTested}' defined in ${whereIsLanguageDefined}`, () => {
                        ...
                        function startNewSession() {
                            ...
                            when(mockedVSCodeNamespaces.extensions.getExtension(anything())).thenCall((extensionId) => {
                                if (extensionId === 'vscode.ipynb') {
                                    return {
                                        exports: {
                                            dropCustomMetadata: !useCustomMetadata
                                        }
                                    };
                                } else {
                                    return;
                        ...
                        function createNotebook(language: string) {
                            ...
                            when(notebook.metadata).thenReturn(
                                useCustomMetadata ? ({ custom: notebookContent } as any) : notebookContent
                            );
