...
export class ConfigurePythonNotebookTool {
    ...
    async invoke(notebook: NotebookDocument, token: CancellationToken) {
        if (!extensions.getExtension(PythonExtensionId) && !this.controllerRegistration.getSelected(notebook)) {
        ...
        if (preferredController && !this.controllerRegistration.getSelected(notebook)) {
                ...
                confirmationMessages: {
                    ...
                    message: l10n.t(
                        [
                            ...
                            'Once installed a Pyton Kernel will be selected and started for the notebook. '
                        ].join('  \n')
                    )
                },
