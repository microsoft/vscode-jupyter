...
suite('DataScience - Jupyter Interpreter State', () => {
    ...
    test('If memento is set (for subsequent sesssions), return true', async () => {
        const uri = Uri.file('jupyter.exe');
        when(memento.get<Uri | undefined>(anything(), undefined)).thenReturn(uri);
    ...
    test('Get python path from memento', async () => {
        ...
        assert.equal(selected.selectedPythonPath?.fsPath, uri.fsPath);
    });
});
