...
export function generateIdFromRemoteProvider(options: { id: string; handle: string }) {
    // eslint-disable-next-line
    return `${Identifiers.REMOTE_URI}?${Identifiers.REMOTE_URI_ID_PARAM}=${options.id}&${
        Identifiers.REMOTE_URI_HANDLE_PARAM
    }=${encodeURI(options.handle)}`;
}
